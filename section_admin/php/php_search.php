...
$empId = $_POST["empId"];
$key = $_POST["key"];
$sql = mysqli_query($conn,"SELECT * FROM tbl_file WHERE emp_id='$empId' AND file_name LIKE '%$key%'");
...
if(mysqli_num_rows($sql)==0){
	...
	while($row = mysqli_fetch_array($sql)){
		...
		if($row["file_updated"]=='1'){
			...
			<td><div style=\"white-space: nowrap; width: 90%; overflow: hidden;text-overflow: ellipsis;\">".$row["file_name"]."</div></td>";
