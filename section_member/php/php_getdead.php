...
if(mysqli_num_rows($sql)!=0){
	...
	$arrDateMonth = date('F j, Y',strtotime($finDate));
	if($time[0] >12){
		echo $arrDateMonth."‼".$day." at ".($time[0]-12).":".$time[1]." PM";	
	}else{
		echo $arrDateMonth."‼".$day." at ".($time[0]-12).":".$time[1]." AM";	
	}
}else{
	echo "(Deadline not set)";
}
echo "<script>console.log('".$arrDateMonth."');</script>";
?>